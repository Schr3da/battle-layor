  ...
  public update(dt: number) {
    ...
    if (this.controls.getStateForKey(SupportedKeys.Up)) {
      const xValue = this.map[
        Math.floor(this.position.x + this.direction.x * this.moveSpeed * 4)
      ...
      const yValue = this.map[Math.floor(this.position.x)][
        Math.floor(this.position.y + this.direction.y * this.moveSpeed * 4)
      ];
    ...
    if (this.controls.getStateForKey(SupportedKeys.Right)) {
      ...
      this.direction.x =
        this.direction.x * Math.cos(-this.rotSpeed) -
        this.direction.y * Math.sin(-this.rotSpeed);
      this.direction.y =
        this.prevDirX * Math.sin(-this.rotSpeed) +
        this.direction.y * Math.cos(-this.rotSpeed);
      this.prevPlaneX = this.plane.x;
      this.plane.x =
        this.plane.x * Math.cos(-this.rotSpeed) -
        this.plane.y * Math.sin(-this.rotSpeed);
      this.plane.y =
        this.prevPlaneX * Math.sin(-this.rotSpeed) +
        this.plane.y * Math.cos(-this.rotSpeed);
    }
